defmodule BoxTest do
  ...
  test "happy path with renaming" do
    Box.Mock.mock(fn
      {:files, "folder_id"} -> {:ok, []}
      {:upload, "folder_id", "foo.pdf", "/tmp/foo.pdf"} -> {:ok, "box_id_1"}
      {:upload, "folder_id", "foo (1).pdf", "/tmp/foo.pdf"} -> {:ok, "box_id_2"}
    end)
  ...
  test "box is down" do
    Box.Mock.mock(fn
      ...
      {:upload, "down_folder_id", "foo.pdf", "/tmp/foo.pdf"} -> {:error, :box_error}
    end)
